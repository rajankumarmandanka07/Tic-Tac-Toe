...
WHERE (c.LEN * c.WIDTH * c.HEIGHT) = (
    ...
    FROM CARTON c2
    WHERE (c2.LEN * c2.WIDTH * c2.HEIGHT) > (
        SELECT SUM(p.LEN * p.WIDTH * p.HEIGHT * oi.PRODUCT_QUANTITY) AS TOTAL_ITEM_VOLUME
        FROM ORDER_ITEMS oi
        JOIN PRODUCT p ON oi.PRODUCT_ID = p.PRODUCT_ID
        WHERE oi.ORDER_ID = 10006
...
SELECT PRODUCT_CLASS_DESC, TOTAL_QUANTITY, TOTAL_VALUE
FROM TotalQuantities
WHERE TOTAL_QUANTITY = (SELECT MAX(TOTAL_QUANTITY) FROM TotalQuantities);
